...
if jj
    ...
    c(1)=rectangle('position',[238.5,172.5,15,15],'curvature',[1,1],'facecolor',rand(1,3));
    c(2)=rectangle('position',[271-7.5,229-7.5,15,15],'curvature',[1,1],'facecolor',rand(1,3));
    c(3)=rectangle('position',[306-7.5,273-7.5,15,15],'facecolor',rand(1,3),'curvature',[1,1]);
    c(4)=rectangle('position',[416-7.5,328-7.5,15,15],'facecolor',rand(1,3),'curvature',[1,1]);
    %% Making the path
    ...
    if lm<4
        ...
        if lm>=2
            jj=jj+1;
            play(victor);
            text (-0.1,0.8,['    You have got ',num2str(lm),' coins    ';'You have Unlocked next level';' Do you want to play again? '],'fontsize',16);
        else
            play(lost);
            text (-0.1,0.8,['     You have got ',num2str(lm),' coins    ';'You didnt Unlocked next level';'  Do you want to play again? '],'fontsize',16);
        end
        ...
        axis off
        play(victor);
        text (0,0.7,['Congrats! You have unlocked bonus level.';'          Press any key to continue.    '],'fontsize',17);
        pause
        bonuslevel2
    end
